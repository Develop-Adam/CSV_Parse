...
def build_parser():
    ...
    p.add_argument("--nest-qa", action="store_true",
...
def apply_contains(df, rules):
    for rule in rules:
        ...
        df = df[df[col].astype(str).str.replace(NBSP, " ", regex=False).str.strip().str.lower().str.contains(sub, na=False)]
    return df
...
def main(argv=None):
    ...
    # Load as text; your file is comma-delimited with quoted fields
    ...
    if args.nest_qa:
        ...
        static_cols = [c for c in df.columns if not c.lower().startswith("question ") and not c.lower().startswith("answer ")]
